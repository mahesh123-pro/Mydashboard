    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <GlassCard className="p-6 lg:col-span-2">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {PREP.map((p) => (
              <div key={p.name} className="rounded-tile border border-border bg-surface p-4">
                <div className="grid size-9 place-items-center rounded-field" style={{ background: `${p.color}1f`, color: p.color }}>
                  <p.icon className="size-4" />
                </div>
                <div className="mt-3 text-sm font-medium">{p.name}</div>
                <ProgressBar value={p.pct} color={p.color} gradient={false} label={p.name} className="mt-2" />
                <div className="mt-1 text-2xs text-muted-2 tabular">{p.pct}% ready</div>
              </div>
            ))}
          </div>
        </GlassCard>
        ...
        <GlassCard className="p-6">
          ...
          <div className="space-y-2">
            {RESUMES.map((r) => (
              <div key={r.name} className="rounded-field border border-border bg-surface p-3">
                <div className="text-sm font-medium">{r.name}</div>
                <div className="text-2xs text-muted-2">{r.note}</div>
              </div>
            ))}
          </div>
        </GlassCard>
      ...
      <GlassCard className="p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {SKILLS.map((sk) => (
            <span key={sk} className="rounded-full border border-border bg-surface px-3 py-1.5 text-sm text-muted transition-colors hover:text-foreground hover:border-border-strong">
              {sk}
            </span>
          ))}
        </div>
