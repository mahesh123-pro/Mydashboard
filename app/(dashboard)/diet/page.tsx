...
// Carbs/fat have no editable target in Settings, so they stay fixed here.
// Calories, protein and water follow the user's goals from the store.
...
export default function DietPage() {
  ...
  const g = useStore((x) => x.healthGoals);
  ...
  const macros = [
    { icon: Flame, label: "Calories", value: kcal, goal: g.calories, unit: "kcal", color: "#f59e0b" },
    { icon: Egg, label: "Protein", value: protein, goal: g.protein, unit: "g", color: "#22c55e" },
    { icon: Wheat, label: "Carbs", value: carbs, goal: CARBS_GOAL, unit: "g", color: "#14b8a6" },
    { icon: Droplet, label: "Fats", value: fat, goal: FAT_GOAL, unit: "g", color: "#059669" },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
        {macros.map((m) => {
          ...
          return (
            <GlassCard key={m.label} hover className="flex items-center gap-4 p-6">
        ...
        <GlassCard className="p-6">
          ...
          <div className="relative">
            ...
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-2xs text-muted-2">kcal today</span>
            </div>
          </div>
          <div className="mt-2 flex justify-center gap-4">
            {distribution.map((d) => (
              <span key={d.name} className="inline-flex items-center gap-1.5 text-2xs text-muted">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <GlassCard className="flex flex-col p-6">
          ...
          <div className="mt-4 flex flex-1 flex-col items-center justify-center">
            <ProgressRing value={Math.round(clamp(water / g.water) * 100)} size={120} stroke={11} from="#14b8a6" to="#10b981">
              <div className="text-2xl font-bold tabular">{water}</div>
              <div className="text-2xs text-muted-2">of {g.water}</div>
            </ProgressRing>
            <div className="mt-4 flex items-center gap-2">
              <button onClick={() => addWater(-1)} className="grid size-9 place-items-center rounded-field border border-border-strong text-muted cursor-pointer hover:text-foreground">
              ...
              <button
                ...
                className="inline-flex h-9 items-center gap-1.5 rounded-field bg-accent/20 px-4 text-sm font-medium text-accent cursor-pointer"
              >
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {supplements.map((sup) => (
              <button
                ...
                className={cn(
                  "flex items-center gap-3 rounded-tile border p-3.5 text-left transition-colors",
                  sup.taken ? "border-success/40 bg-success/10" : "border-border bg-surface hover:border-border-strong",
                )}
              >
                <span
                  className={cn(
                    "grid size-5 place-items-center rounded-control border",
                ...
                <div className="flex-1">
                  ...
                  <div className="text-2xs text-muted-2">{sup.dose}</div>
                </div>
              </button>
            ))}
          </div>
