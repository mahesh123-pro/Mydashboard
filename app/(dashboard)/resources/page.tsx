        ...
        {SECTIONS.map((s) => (
          <GlassCard key={s.title} className="p-6">
            <div className="mb-4 flex items-center gap-2.5">
              <div className="grid size-9 place-items-center rounded-xl" style={{ background: `${s.color}1f`, color: s.color }}>
            ...
            <div className="space-y-2">
              {s.items.map((it) => (
                <div key={it.name} className="group flex items-center gap-3 rounded-xl border border-border bg-white/[0.02] p-3 transition-colors hover:border-border-strong">
                  <Link2 className="size-4 shrink-0 text-muted-2" />
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm">{it.name}</div>
                    <div className="text-[11px] text-muted-2">{it.sub}</div>
                  </div>
