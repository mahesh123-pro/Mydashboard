    ...
    <div className="space-y-6">
        ...
        {TRACKS.map((t) => (
          <GlassCard key={t.name} hover className="p-6">
            <div className="mb-4 flex items-center gap-2.5">
              <div className="grid size-9 place-items-center rounded-field" style={{ background: `${t.color}1f`, color: t.color }}>
            ...
            <div className="space-y-3">
              {t.skills.map((sk) => (
                <div key={sk.name}>
                  ...
                  <ProgressBar value={sk.pct} color={t.color} label={sk.name} />
                </div>
              ))}
            </div>
          </GlassCard>
      ...
      <GlassCard className="p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {CERTS.map((c) => (
            <div key={c.name} className="rounded-tile border border-border bg-surface p-4">
              <div className="grid size-9 place-items-center rounded-field" style={{ background: `${c.color}1f`, color: c.color }}>
                <Award className="size-4" />
              </div>
              <div className="mt-3 text-sm font-medium">{c.name}</div>
              <div className="mt-1 text-2xs text-muted-2">{c.status}</div>
            </div>
          ))}
        </div>
      </GlassCard>
